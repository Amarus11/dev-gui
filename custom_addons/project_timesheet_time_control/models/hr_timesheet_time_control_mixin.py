...
class HrTimesheetTimeControlMixin(models.AbstractModel):
    ...
    running_timer_date_time = fields.Datetime(
        string="Running Timer Start",
        compute="_compute_running_timer_date_time",
    )
    ...
    def _compute_running_timer_date_time(self):
        """Find the start time of the running timer for this record."""
        ...
        for line in running_lines:
            rec_id = line[related_field].id
            if rec_id not in running_by_record:
                running_by_record[rec_id] = line.date_time
        for record in self:
            record.running_timer_date_time = running_by_record.get(
                record.id, False
            )
